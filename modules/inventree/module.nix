...
let
  ...
  opt = options.services.inventree;
  ...
  config = mkIf cfg.enable ({
      ...
      serviceConfig = {
        ...
        ExecStartPre =
          "+${pkgs.writers.writeBash "inventree-setup" ''
            ...
            ${inputs.inventree.packages.${system}.inventree-invoke}/bin/inventree-invoke migrate

            echo "Ensuring static files are populated"
            ${inputs.inventree.packages.${system}.inventree-invoke}/bin/inventree-invoke static

            echo "Setting up users"
            cat ${usersFile} | \
              ${inputs.inventree.packages.${system}.inventree-refresh-users}/bin/inventree-refresh-users
          ''}";
        ExecStart = ''
          ${inputs.inventree.packages.${system}.inventree-server}/bin/inventree-server -b ${cfg.serverBind}
        '';
      };
    };
    systemd.services.inventree-cluster = {
      ...
      serviceConfig = {
        ...
        Group = defaultGroup;
        ExecStart = ''
          ${inputs.inventree.packages.${system}.inventree-cluster}/bin/inventree-cluster
        '';
      };
    };
  });
